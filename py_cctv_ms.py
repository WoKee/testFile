...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def get_m3u8(self,urlTxt):
		...
		if hdRsp == 200:
			...
			maxVideo = '2000'
			hdUrl = hdUrl.replace('4000', maxVideo)
			hdRsp = self.TestWebPage(urlStr=hdUrl,header=self.header)
			if hdRsp == 200:
				return hdUrl
			else:
				maxVideo = '1200'
				hdUrl = hdUrl.replace('2000', maxVideo)
				hdRsp = self.TestWebPage(urlStr=hdUrl,header=self.header)
				if hdRsp == 200:
					return hdUrl
				else:
					maxVideo = '850'
					hdUrl = hdUrl.replace('1200', maxVideo)
					hdRsp = self.TestWebPage(urlStr=hdUrl,header=self.header)
					if hdRsp == 200:
						return hdUrl
					else:
						maxVideo = '450'
						hdUrl = hdUrl.replace('850', maxVideo)
						return hdUrl
		return url
	#搜索
